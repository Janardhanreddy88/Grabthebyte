    ...
    <AnimatePresence>
      {totalItems > 0 && (
        <motion.div initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 80, opacity: 0 }}
          transition={{ type: "spring", stiffness: 300, damping: 25 }} className="fixed bottom-0 left-0 right-0 p-3 lg:hidden z-50">
          <button onClick={() => navigate('/checkout')}
            className="w-full flex items-center justify-between p-3.5 rounded-2xl bg-secondary text-secondary-foreground active:scale-[0.98] transition-transform fab-shadow">
