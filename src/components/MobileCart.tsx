          ...
          <button onClick={() => navigate('/checkout')}
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-11 h-11 rounded-xl bg-secondary-foreground/10 flex items-center justify-center"><ShoppingBag size={20} /></div>
                <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-canteen-warning text-canteen-dark rounded-full flex items-center justify-center text-[11px] font-bold">{totalItems}</span>
              </div>
              <div className="text-left">
                <span className="font-bold text-base">{totalItems} item{totalItems > 1 ? 's' : ''}</span>
