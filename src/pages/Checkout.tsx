        ...
        <div className="max-w-2xl mx-auto w-full px-4 py-5 pb-36 space-y-5">
              ...
              {cart.map((item) => (
                <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 flex gap-3.5">
                  <div className="relative h-18 w-18 flex-shrink-0">
                    ...
                    <div className="absolute -top-1.5 -right-1.5 h-5 min-w-[1.25rem] px-1 rounded-full bg-foreground text-background text-[10px] font-bold flex items-center justify-center">x{item.quantity}</div>
                  </div>
          ...
          <section className="space-y-3">
            ...
            <div className="bg-card rounded-xl shadow-sm border border-border/50 p-4 space-y-3">
              ...
              <div className="flex justify-between items-center"><span className="font-bold text-base">To Pay</span><span className="font-bold text-lg text-primary">₹{totalPrice}</span></div>
            </div>
          </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-xl border-t border-border/40 z-50">
        <div className="max-w-2xl mx-auto flex gap-4 items-center">
          ...
          <motion.div className="flex-[1.5]" whileTap={{ scale: 0.98 }}>
            <Button className="w-full h-12 rounded-xl text-sm font-bold shadow-lg shadow-primary/20" onClick={handlePlaceOrder} disabled={isLoading}>
              {isLoading ? (
                <span className="flex items-center gap-2"><Loader2 className="animate-spin" size={16} />{isCreating ? "Creating Order..." : "Checking..."}</span>
              ) : (
                <span className="flex items-center gap-2">Proceed to Pay <ArrowRight size={16} /></span>
              )}
            </Button>
          </motion.div>
        </div>
