    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <Input id={id} type={type} placeholder={placeholder} value={value} onChange={onChange} disabled={disabled}
          className={`h-9 pl-9 text-sm rounded-xl ${disabled ? 'bg-muted/50 text-muted-foreground cursor-not-allowed' : ''}`} />
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background pb-16">
      <header className="sticky top-0 z-50 bg-card/90 backdrop-blur-md border-b border-border">
        <div className="flex items-center gap-3 p-3 max-w-lg mx-auto">
          <button onClick={() => navigate('/menu')} className="w-7 h-7 rounded-full bg-muted flex items-center justify-center"><ArrowLeft size={14} /></button>
      ...
      <main className="max-w-lg mx-auto p-4 space-y-5">
        <div className="flex flex-col items-center py-4">
          <div className="w-16 h-16 rounded-full bg-primary flex items-center justify-center text-primary-foreground text-xl font-bold mb-2">{getInitials(fullName)}</div>
          <p className="text-[10px] text-muted-foreground">Your profile</p>
        </div>
        ...
        <Button onClick={handleSave} disabled={saving} className="w-full h-9 text-sm font-bold rounded-xl gap-1.5">
